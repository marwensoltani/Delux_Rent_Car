...
<head>
	...
	<title>System de Location des Voitures - Recherche intervenation par intervenant</title>
    					...
    					<div class="col-md-12">
                            <div class="card">
                                <div class="card-header text-center" data-background-color="orange">
                                    <h4 class="title">Recherche intervenation par intervenant</h4>
	

									<?php
									$intervenant=$_POST['intervenant'];
									$requete = "select nomindividu, prenomindividu from individu where idindividu='".$intervenant."'";
									$resultat = mysqli_query($connect,$requete);
									$row = mysqli_fetch_row($resultat);
									?>
									<p class="categorie">Recherche d'intervention de <?php echo $row[0].' '.$row[1]?></p>   
		                                </div>
							...
							//$date_probleme=$d.'/'.$m.'/'.$y; // date au format fran?
							$dateintervention=$row[3];
							// $date_intervention est une valeur r?p?e au format YYYY-MM-DD
							//list($y,$m,$d) = explode('-', $date_intervention);
							//$date_intervention=$d.'/'.$m.'/'.$y; // date au format fran?s
